...
# Abstract
추천 시스템은 실시간 상호 작용 중에 정보, 제품 또는 서비스에 대한 개인화 된 권장 사항을 만드는 문제에 지식 검색 기술을 적용한다. 이러한 시스템, 특히 k-최근접 이웃 협업 필터링 기반 시스템은 웹에서 광범위하게 쓰이고 있다. 최근 몇 년 동안 사용 가능한 정보의 양과 웹 사이트 방문자 수가 엄청나게 증가함에 따라 추천 시스템에 몇 가지 이슈가 생겼다. 고품질 추천 생성, 수백만명의 사용자, 항목에 대해 초당 많은 추천 사항 수행, 데이터 희소성에 직면한 높은 적용 범위 달성 등이 있다. 기존의 협업 필터링 시스템에서 작업량은 시스템 참여자 수에 따라 증가한다. 매우 큰 규모의 문제에서도 고품질 추천을 신속하게 생성할 수 있는 새로운 추천 시스템 기술이 필요하다. 이러한 문제를 해결하기 위해 항목 기반 협업 필터링 기술을 살펴본다. 항목 기반 기술은 먼저 사용자 항목 매트릭스를 분석하여 서로 다른 항목 간의 관계를 식별한 다음 이러한 관계를 사용하여 사용자에 대한 추천 사항을 간접적으로 계산한다. 
