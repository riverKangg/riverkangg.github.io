...
# ABSTRACT
음성 요약은 흐름의 자발성, 유창하지 않음과 같이 텍스트에서 발생하지 않는 문제가 있다. 이 연구는 BERTSum 모델을 대화 데이터에 처음 적용했다. 다양한 주제에 대해 설명된 교육용 영상의 추상 요약을 생성한다. 어휘를 풍부하게 하기위해, 전이 학습을 한다. 서면이나 구어체 영어로 된 대규모 교차 도메인 데이터 셋에서 사전 학습한다. 또한 ASR 시스템의 출력에서 문장 분할과 구두점을 복원하기위해 전처리한다. 결과는 How2, WikiHow에 대한 ROUGE, Content-F1 점수로 평가된다. HowTo100M와 YouTube에서 선별한 데이터 셋에서 무작위로 선택된 요약 세트를 사람이 평가한다. 블라인드 평가 결과, 사람이 작성한 요약과 유사한 수준의 텍스트 유창성과 유용성을 보인다. 스타일과 주제가 다양한 WikiHow 기사에 적용할 때 현재 SOTA를 능가하지만, CNN/DailyMail 데이터 셋에서는 성능 회귀가 없었다. 다양한 스타일과 도메인에서 일반화 할 수 있기 때문에, 인터넷 콘텐츠의 접근성과 검색 가능성을 향상시킬 수 있다.
